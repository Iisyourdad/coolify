<div>
    ...
    <div class="flex flex-col gap-6">
        ...
        <div class="application-settings-form">
            <x-application.settings-section title="Master update report"
                description="Bundle unchanged Coolify, container image, server package, and Traefik update notices into one report.">
                <div class="grid gap-4 lg:grid-cols-3">
                    <x-forms.listbox id="masterUpdateReportEmailNotifications" label="Report delivery"
                        onChange="saveModel" :disabled="!auth()->user()->can('update', $settings)" :options="[
                            ['value' => true, 'label' => 'Enabled'],
                            ['value' => false, 'label' => 'Disabled'],
                        ]" />
                    <x-forms.listbox id="masterUpdateReportFrequency" label="Frequency" onChange="saveModel"
                        :disabled="!auth()->user()->can('update', $settings)" :options="[
                            ['value' => 'daily', 'label' => 'Daily'],
                            ['value' => 'weekly', 'label' => 'Weekly'],
                        ]" />
                    @if ($masterUpdateReportFrequency === 'weekly')
                        <x-forms.listbox id="masterUpdateReportDay" label="Send day" onChange="saveModel"
                            :disabled="!auth()->user()->can('update', $settings)" :options="[
