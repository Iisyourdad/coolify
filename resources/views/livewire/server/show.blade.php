    ...
    <div
        ...
        <div class="flex w-full min-w-0 flex-col gap-6">
            ...
            @if ($server->proxyType() === \App\Enums\ProxyTypes::TRAEFIK->value)
                <x-application.settings-section id="server-master-domain-routing-section" title="Master domain routing"
                    helper="Forward domains that point to this server to resources hosted on other servers.">
                    ...
                    @else
                        <x-forms.checkbox canGate="update" :canResource="$server" live
                            id="isMasterDomainRouterEnabled" label="Use as the master domain router"
                            helper="Enable this on exactly one server when all domains point here and traffic should be forwarded to resources on other servers, for example over VPN or WireGuard."
                            :disabled="$isValidating" />
                    @endif
                </x-application.settings-section>
            @endif
