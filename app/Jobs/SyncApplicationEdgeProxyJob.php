...
class SyncApplicationEdgeProxyJob implements ShouldBeEncrypted, ShouldQueue
{
    ...
    {
        return [(new WithoutOverlapping('sync-application-edge-proxy-'.$this->application->uuid))->expireAfter(120)->dontRelease()];
    }
    ...
    {
        try {
            $routeWarnings = app(EdgeProxyRemoteRouteService::class)->syncApplication($this->application);
        ...
        try {
            $portForwardWarnings = app(EdgeProxyRemotePortForwardService::class)->syncApplication($this->application);
