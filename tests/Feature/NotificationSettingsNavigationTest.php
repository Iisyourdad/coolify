...
it('keeps master update report controls in the redesigned notification settings', function () {
    $email = file_get_contents(resource_path('views/livewire/notifications/email.blade.php'));
    $serverPatches = file_get_contents(resource_path('views/livewire/server/security/patches.blade.php'));

    expect($email)
        ->toContain('title="Master update report"')
        ->toContain('id="masterUpdateReportEmailNotifications"')
        ->toContain('id="masterUpdateReportFrequency"')
        ->toContain('id="masterUpdateReportDay"')
        ->toContain('<x-notification.settings-layout>')
        ->not->toContain('<x-notification.navbar');

    expect($serverPatches)
        ->toContain('Server package updates are')
        ->toContain('included in the master update report')
        ->toContain('<x-server.sidebar :server="$server" activeMenu="security" />');
});
...
it('renders settings section descriptions or helper text in the header', function () {
    $section = file_get_contents(resource_path('views/components/application/settings-section.blade.php'));

    expect($section)
        ->toContain('filled($description ?? $helper)')
        ->toContain(':helper="$description ?? $helper"');
});
