...
it('opens a selected service application with both action menus', function () {
    $this->withoutExceptionHandling();
    $this->team->update(['show_boarding' => false]);
    Cache::flush();
    Queue::fake();
    $this->webApp->update(['status' => 'running:healthy']);

    $this->get(route('project.service.index', [
        'project_uuid' => $this->project->uuid,
        'environment_uuid' => $this->environment->uuid,
        'service_uuid' => $this->service->uuid,
        'stack_service_uuid' => $this->webApp->uuid,
    ]))
        ->assertOk()
        ->assertSeeLivewire(Heading::class)
        ->assertSeeHtml('id="service-mobile-actions"')
        ->assertSeeHtml('id="service-desktop-actions"');
});
