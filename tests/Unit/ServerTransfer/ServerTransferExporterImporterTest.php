...
test('export omits instance-bound master domain router status', function () {
    Queue::fake();
    $this->server->proxy->set('type', ProxyTypes::TRAEFIK->value);
    $this->server->save();
    $this->server->settings->update(['is_master_domain_router_enabled' => true]);

    $bundle = $this->exporter->export($this->server);

    expect($this->server->settings->fresh()->is_master_domain_router_enabled)->toBeTrue()
        ->and($bundle['server']['settings'])->not->toHaveKey('is_master_domain_router_enabled');
});

test('import clears legacy master domain router status without replacing the target master', function () {
    Queue::fake();

    $bundle = $this->exporter->export($this->server);
    $bundle['server']['settings']['is_master_domain_router_enabled'] = true;

    $targetMaster = Server::factory()->create([
        'team_id' => $this->team->id,
        'private_key_id' => $this->privateKey->id,
        'ip' => '10.55.0.11',
        'proxy' => ['type' => ProxyTypes::TRAEFIK->value],
    ]);
    $targetMaster->settings->update(['is_master_domain_router_enabled' => true]);
    ...
    $importedServer = Server::where('uuid', $result['server_uuid'])->firstOrFail();

    expect($importedServer->settings->is_master_domain_router_enabled)->toBeFalse()
        ->and($targetMaster->settings->fresh()->is_master_domain_router_enabled)->toBeTrue();
});
